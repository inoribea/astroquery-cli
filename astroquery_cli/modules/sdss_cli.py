...
def get_app():
    ...
    app = typer.Typer(
        name="sdss",
        help=builtins._("Query the Sloan Digital Sky Survey (SDSS) database."),
    ...
    @app.command(name="query", help=builtins._("Perform a query on SDSS."))
    @global_keyboard_interrupt_handler
    def query_sdss(
        ctx: typer.Context,
        ...
        objtype: Optional[str] = typer.Option(None, help=builtins._("Object type (e.g., 'STAR', 'GALAXY', 'QSO').")),
        specobjid: Optional[int] = typer.Option(None, help=builtins._("Spectroscopic object ID.")),
        plate: Optional[int] = typer.Option(None, help=builtins._("Plate number for spectroscopic data.")),
        mjd: Optional[int] = typer.Option(None, help=builtins._("Modified Julian Date for spectroscopic data.")),
        fiberid: Optional[int] = typer.Option(None, help=builtins._("Fiber ID for spectroscopic data.")),
        ...
        try:
            ...
            if ra is not None and dec is not None:
                ...
                results = SDSS.query_region(ra=ra, dec=dec, radius=radius, objtype=objtype)
            elif specobjid is not None:
                results = SDSS.query_specobjid(specobjid)
            elif plate is not None and mjd is not None and fiberid is not None:
                results = SDSS.query_fiberid(plate=plate, mjd=mjd, fiberid=fiberid)
            else:
                console.print(_("[red]Error: Please provide coordinates (--ra, --dec, --radius), or --specobjid, or --plate, --mjd, and --fiberid.[/red]"))
                raise typer.Exit(code=1)
